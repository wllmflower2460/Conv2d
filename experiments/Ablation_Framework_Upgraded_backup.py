...
if __name__ == "__main__":
    print("Ablation Framework (Upgraded) — synthetic smoke test")
    set_all_seeds(42)
    Xtr = torch.randn(256, 9, 2, 100)
    ytr = torch.randint(0, 12, (256,))
    Xva = torch.randn(64, 9, 2, 100)
    yva = torch.randint(0, 12, (64,))
    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(Xtr, ytr), batch_size=32, shuffle=True)
    val_loader   = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(Xva, yva), batch_size=32, shuffle=False)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    runner = AblationRunner(train_loader, val_loader, device=device, output_dir="ablation_results")
    results = runner.run_ablation(epochs=3, seed=123)
    runner.generate_report(results)
    print("✅ Done. See ablation_results/ for JSON, CSV, and report.")
