#!/usr/bin/env python3
"""
Ablation study with verbose VQ evaluation logging to diagnose codebook collapse.
Tracks per-batch statistics during evaluation to identify when/why collapse occurs.
"""
...
class AblationModel(nn.Module):
    ...
    def forward(self, x, verbose_eval: bool = False) -> Dict[str, Any]:
        ...
        Args:
            ...
            verbose_eval: If True, return detailed VQ statistics
        
        Returns:
            Dictionary with logits and optional loss components
        """
        batch_size = x.size(0)
        ...
        if self.vq is not None:
            # Reshape for VQ: (B, 128) -> (B, 64, 1, 2)
            features = features.view(batch_size, self.config.code_dim, -1).unsqueeze(2)
            features, vq_loss, vq_info = self.vq(features)
            features = features.squeeze(2).view(batch_size, -1)
...
def initialize_codebook_from_data(model: AblationModel, data_loader: DataLoader, device: torch.device):
    ...
    with torch.no_grad():
        for x, _ in data_loader:
            ...
            embeddings.append(enc_out.cpu())
            if len(embeddings) * x.size(0) >= model.vq.num_codes * 2:
                break
...
def evaluate_verbose(model: nn.Module, loader: DataLoader, device: torch.device, 
    ...
    """Evaluate with verbose VQ statistics."""
    model.eval()
    ...
    print("\n" + "="*80)
    print("VERBOSE EVALUATION - Tracking VQ statistics per batch")
    print("="*80)
    ...
    if config.use_vq and all_perplexities:
        ...
        print("\n" + "="*80)
        print("EVALUATION SUMMARY - VQ Statistics")
        print("="*80)
...
def main():
    ...
    # Define configurations - focus on VQ variants for debugging
    configs = [
    ...
    output_dir = f"ablation_vq_verbose_{timestamp}"
    ...
    # Print comparison
    print("\n" + "="*80)
    print("ABLATION COMPARISON - VQ Codebook Analysis")
    print("="*80)
    ...
    # Save detailed VQ analysis for the worst performer
    worst_vq_config = min(
        [k for k in results.keys() if "vq" in k],
        key=lambda x: results[x]["accuracy"]
    )
    
    with open(f"{output_dir}/vq_detailed_analysis_{worst_vq_config}.json", "w") as f:
        json.dump(results[worst_vq_config].get("vq_details", {}), f, indent=2)
    
    print(f"Detailed VQ analysis for {worst_vq_config} saved to {output_dir}/")
