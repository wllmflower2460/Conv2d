...
class CVATToSLEAPConverter:
    ...
    def __init__(self):
        ...
        self.connections = [
            ...
            ("right_eye", "right_ear"), ("nose", "neck"), ("neck", "spine_mid"),
            # Front legs
            ("neck", "left_front_shoulder"), ("left_front_shoulder", "left_front_elbow"),
            ("left_front_elbow", "left_front_wrist"), ("left_front_wrist", "left_front_paw"),
            ("neck", "right_front_shoulder"), ("right_front_shoulder", "right_front_elbow"),
            ("right_front_elbow", "right_front_wrist"), ("right_front_wrist", "right_front_paw"),
            # Back legs
            ("spine_mid", "left_back_shoulder"), ("left_back_shoulder", "left_back_elbow"),
            ("left_back_elbow", "left_back_wrist"), ("left_back_wrist", "left_back_paw"),
            ("spine_mid", "right_back_shoulder"), ("right_back_shoulder", "right_back_elbow"),
            ("right_back_elbow", "right_back_wrist"), ("right_back_wrist", "right_back_paw"),
            # Tail
            ("spine_mid", "tail_base"), ("tail_base", "tail_mid"), ("tail_mid", "tail_tip")
        ]
    ...
    def create_skeleton(self) -> None:
        ...
        print("🦴 Creating DataDogs animal skeleton for SLEAP...")
    ...
    def parse_cvat_xml(self, cvat_file: str) -> List[Dict]:
        ...
        for image_elem in root.findall('.//image'):
            ...
            # Look for skeleton/keypoint annotations
            skeletons = []
            
            # Parse skeleton elements (preferred)
            ...
            # Parse individual points if no skeletons found
            if not skeletons:
                points = image_elem.findall('.//points')
                if points:
                    skeleton_data = self.parse_individual_points(points)
                    if skeleton_data:
                        skeletons.append(skeleton_data)
        ...
        print(f"📊 Parsed {len(annotations)} annotated images with skeletons")
        return annotations
    ...
    def parse_individual_points(self, points_elements) -> Optional[Dict]:
        """Parse individual CVAT points when skeleton not used"""
        keypoints = {}
        ...
        for point_elem in points_elements:
            ...
            if ',' in points_str:
                coords = points_str.split(';')[0].split(',')
                if len(coords) >= 2:
                    x, y = float(coords[0]), float(coords[1])
    ...
    def map_cvat_label_to_node(self, cvat_label: str) -> Optional[str]:
        ...
        label_mapping = {
            'left_front_leg_shoulder': 'left_front_shoulder',
            'right_front_leg_shoulder': 'right_front_shoulder',
            'left_back_leg_shoulder': 'left_back_shoulder', 
            'right_back_leg_shoulder': 'right_back_shoulder',
            'spine_middle': 'spine_mid',
            'tail_end': 'tail_tip',
            'tail_middle': 'tail_mid'
        }
        
        return label_mapping.get(cvat_label.lower())
